...
describe Address4 do
  describe "with new" do
    let(:address) { Address4.new(street: "12 Smith Street", city_name: "Burpengary") }
    it { address.postcode.must_equal "4505" }
    it { address.valid?.must_equal true }
  end

  describe "with create" do
    let(:address) { Address4.new(street: "12 Smith Street", city4: [city4(:burp)]) }
    it { address.postcode.must_equal "4505" }
  ...
  describe "with new" do
    let(:address) { Address4.new(street: "12 Smith Street", city_name: "Beverly Hills") }
